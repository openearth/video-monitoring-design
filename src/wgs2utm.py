...
def wgs2utm(Lat,Lon,utmzone,utmhemi):
...
#-------------------------------------------------------------------------
    ...
    if (n1!=n2):
        ...
        return
    if exist('utmzone','var') and exist('utmhemi','var'):
        n3=np.size(utmzone);
        n4=np.size(utmhemi);
        if (np.sort(n3)~=np.sort(n4)):
            error('utmzone and utmhemi should have same size');
            return
        
        if max(n3)!=1 and max(n3)!=max(n1):
    ...
    if exist('utmzone','var') and exist('utmhemi','var'):
        ...
        n4=np.size(utmhemi);
        if n3==[1 1]:
            utmzone = utmzone.*np.ones(np.size(Lat));
            utmhemi = char(utmhemi.*np.ones(np.size(Lat)));
    ...
    lon0 = Lon0*np.pi/180;      # in radians
